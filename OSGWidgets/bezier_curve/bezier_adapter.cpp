...
vector<Bezier::Point> BezierAdapter::getBezierPoints(int size)
{
	vector<Bezier::Point> result;
    if(m_originControlPoints.size()<2)
        return result;

    auto n = m_originControlPoints.size()-1;
    ...
    for(int i = 0; i < n; ++i)
    {
        for (int j = 0; j < size; ++j) {
            auto per = static_cast<double>(j) / static_cast<double>(size);
            Bezier::Point p = cubicBezier[i].valueAt(per);
            result.push_back(p);
        }
    }
    return result;
